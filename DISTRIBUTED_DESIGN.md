...
`sql/05_partitioning_demo.sql` *(Note: script not yet committed, captured output is in performance_results/; will be re-added.)* It creates a rollback-only partitioned clone from the currently loaded data and displays comparable `EXPLAIN (ANALYZE, BUFFERS)` plans.
