...
`sql/04_validation.sql` *(Note: script not yet committed, captured output is in performance_results/; will be re-added.)* It provides repeatable database tests, rolls back all test data, and verifies:
...
```bash
# (pending re-add) psql -v ON_ERROR_STOP=1 -d fraudguard -f sql/04_validation.sql
```
...
`sql/05_partitioning_demo.sql` *(Note: script not yet committed, captured output is in performance_results/; will be re-added.)* It builds a monthly range-partitioned clone of the transaction table, copies the currently loaded rows, and compares a one-month report against the unpartitioned table. The entire operation is rolled back.

Run:

```bash
psql -v ON_ERROR_STOP=1 -d fraudguard \
  -f sql/05_partitioning_demo.sql \  # pending re-add
